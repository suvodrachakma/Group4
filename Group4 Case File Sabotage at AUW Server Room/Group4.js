...
function interrogate(name) {
  ...
  if (name === 'Mittra') {
    body = '"I was studying in the common room late! Yes, I\'m left-handed and currently I am studying CS, but I would never sabotage AUW servers right before our exam!"';
  } else if (name === 'Lomi') {
...
function accuse() {
  ...
  if (selected === 'Mittra') {
    showClue('CASE SOLVED!', `Great job, Detective! Mittra (Student 1) used her CS lab training, left-handed tools, and a hardware to cut the AUW server wire at 3:00 AM to delay exam day. Final Score: ${score} points.`);
  } else {
    updateScore(-80);
    showClue('WRONG ACCUSATION!', `Incorrect! ${selected} is right-handed and has a verified dorm alibi for 3:00 AM. You lost 80 points. Keep searching!`);
  }
}